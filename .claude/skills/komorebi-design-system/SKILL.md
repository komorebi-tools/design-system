...
AI は指示がないと統計的に最も無難なデザインに収束する。以下はコモレビのデザインシステムに反する典型的な収束パターン。
...
| 装飾目的の丸いグラデーション blob | コモレビの世界観は「透明感、水彩、やさしい」。blob ではなく余白と shadow で奥行きを出す |
...
- セクション間の余白は --sp-xxl (64px) を基本にリズムをつける。全セクション同じ余白は単調になる
...
画面が正しく動くこと。これがすべての土台。

- デザインシステムのトークン (色、フォント、余白、シャドウ) を CSS variable 経由で使う。直打ち禁止
- Material Icons を使う。絵文字禁止
- ロゴは実画像を使う。CSS で模造しない
...
- **L1 は AI 主導。** ルールが明確なので AI の出力精度が高い。
- **L2 は協業。** 人間が方向を示し、AI が実装する。
- **L3 は人間主導。** 何がユーザーの記憶に残るかは設計者の感性から生まれる。AI に任せると「想定内の驚き」にしかならない。
