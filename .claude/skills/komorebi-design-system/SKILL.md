...
description: |
  株式会社コモレビのデザインシステム準拠スキル。
  コモレビのウェブサイト、UI、HTMLプロトタイプに加えて、グラフ ･ 図解 ･ 表 ･ スライドを作成 / 編集するときに必ず適用する。
  デザイントークン (色、タイポグラフィ、スペーシング、シャドウ)、アイコンルール (Material Icons 必須、絵文字禁止)、ロゴ使用規定、図表とスライドの規約 (FIGURE.md)、実在する会社情報の参照元を定義。
  トリガー : コモレビ、komorebi、コモレビのサイト、コモレビのUI、コモレビのデザイン、コモレビのHTML、コモレビのウェブ、コモレビのスライド、コモレビの資料、グラフ、図解、チャート、表、ダッシュボード、レポートの図版、食品マーケティングのサイトなど、コモレビに関連するあらゆる UI / Web / 図表 / スライド制作タスク。
---
...
| 章 | 中身 | 適用範囲 |
|---|---|---|
| §1 三層構造 | ブランド基調 ･ カテゴリ配色 ･ 強調 ･ グレー ･ 線 | 媒体を問わない |
...
実物のプレビューは `figure.html`。
規約どおりに作るとどう見えるかを確かめるときに開く。
...
- 既定パレットのまま出さない。
  Google `#4285F4` `#EA4335` `#FBBC04` / matplotlib `#1F77B4` `#FF7F0E` / スプレッドシートのカラースケール `#E67C73`
- 系列ごとに違う色相を割り当てない。
  1つのグラフで使う色は原則2色で、3色目が要るかを先に疑う
- 順序のあるデータは色相を変えず、同系の濃淡で表す
- 軸の見出し ･ 目盛り ･ 値ラベルを必ず付ける。
  何の値かが軸だけで分かる状態にする
- 突出した値のために軸を途中で切らない
- Warning ･ Danger ･ Success をカテゴリの識別に使わない
...
```css <!-- design-check: allow-block NO_HARDCODED_PRIMARY -->
--primary: #6E87B6;
...
色は必ず CSS custom properties 経由で使う。
ハードコードしない。
...
説明文 (.section-desc 等) の行高は最大 `1.7` まで。
それ以上は間延びする。
...
SVG アイコンも原則使わない。
Material Icons に統一することで、サイト全体の一貫性を保つ。

---

## アンチパターン (AI Slop 回避)

AI は指示がないと統計的に最も無難なデザインに収束する。
以下はコモレビのデザインシステムに反する典型的な収束パターン。

**やりがちなミスと正しい対応 :**

| やりがち | 正しい対応 |
|----------|-----------|
| 紫グラデーション背景 | コモレビのカラートークン (--primary: #6E87B6, --accent: #C4A55A) を使う <!-- design-check: allow NO_HARDCODED_PRIMARY --> |
| カード3枚を均等に横並べしただけのレイアウト | 情報の優先度に応じて非対称 ･ 強弱をつける |
| Lucide React や Heroicons のアイコン | Material Icons 以外は禁止 |
| 白背景 + 紫ボタンの SaaS テンプレート風 | --background: #F7F7F7 ベース、ボタンは --primary |
| 装飾目的の丸いグラデーション blob | コモレビの世界観は「透明感、水彩、やさしい」。blob ではなく余白と shadow で奥行きを出す <!-- design-check: allow NO_PERIOD_LINEBREAK --> |
| 汎用ストックフォト風のイラスト | ロゴ ･ 素材はデザインシステムの実画像を使う |

**レイアウトの判断基準 :**

- 対称 ･ 均等配置がデフォルトになっていないか確認する
- 3カラム均等よりも、メイン + サイドのような情報の重み付けを優先する
- セクション間の余白は --sp-xxl (64px) を基本にリズムをつける。
  全セクション同じ余白は単調になる
...
複数画面を作るとき、すべてに同じ品質を求めると破綻する。

品質を3層に分け、画面ごとにどこまで追求するかを最初に決める。
...
- デザインシステムのトークン (色、フォント、余白、シャドウ) を CSS variable 経由で使う。
  直打ち禁止
- Material Icons を使う。
  絵文字禁止
- ロゴは実画像を使う。
...
- 見出しは font-weight: 700、font-feature-settings: "palt" 1, "kern" 1
- 本文は line-height: 1.6、letter-spacing: 0.05em
- 人名、設立年、所在地はソースから取得した実データ
...
- 状態遷移のフィードバックが設計されている (Loading / Success / Error)
- 情報の出し方にリズムがある (プログレッシブディスクロージャー)
- 余白のバランスが意図的に調整されている
- テキストのトーンが統一されている
- エラーハンドリングがユーザーフレンドリー (何が起きて、何をすればいいかが明確)

対象 : ユーザーが最も長く滞在する画面 (ホーム、検索結果、詳細画面など)。
...
- マイクロインタラクションが設計されている
- エモーショナルデザインが意図的に組み込まれている

対象 : オンボーディング、ファーストインプレッション、離脱を防ぐエラー体験など、最も印象に残る画面に限定。

**L3 は人間が方向を決め、AI が実装を手伝う**。

### 画面ごとの品質レベル配分

複数画面の制作に入る前に、各画面の品質レベルを宣言する。

配分の目安 :

| レベル | 割合 | 例 |
|--------|------|-----|
| L3 | 約 10-15% | オンボーディング、ホーム、料金ページ |
| L2 | 約 35-40% | 検索結果、詳細、設定トップ |
| L1 | 約 50% | サブ設定、管理画面、補助的なリスト画面 |

### Wave 方式 (制作の進め方)

1画面ずつ完璧に仕上げるのではなく、全体を波のようになぞる。

**第一波 (L1)** : 全画面の構造を作る。
ワイヤーフレームに近い状態で、全体の構造と画面遷移を確定。
所要時間の目安 : 全体の 30%。

**第二波 (L2)** : 主要画面の体験を仕上げる。
状態遷移、フィードバック、余白バランスを調整。
ここで確立したパターンが他の画面にも波及する。
所要時間の目安 : 全体の 40%。

**第三波 (L3)** : 特定画面に感動を入れる。
マイクロインタラクション、エモーショナルデザイン。
限られた画面に集中投資する。
所要時間の目安 : 全体の 30%。
...
- **L1 は AI 主導**。
  ルールが明確なので AI の出力精度が高い。
- **L2 は協業**。
  人間が方向を示し、AI が実装する。
- **L3 は人間主導**。
  何がユーザーの記憶に残るかは設計者の感性から生まれる。
  AI に任せると「想定内の驚き」にしかならない。
...
- [ ] Lucide React / Heroicons / SVG アイコンが混入していない

### L2 チェック (主要画面)

- [ ] Loading / Success / Error の状態遷移が設計されている
- [ ] 余白のバランスが意図的に調整されている
- [ ] テキストのトーンが統一されている
- [ ] エラーメッセージが「何が起きて、何をすればいいか」を伝えている

### L3 チェック (特定画面)

- [ ] ユーザーの期待を超える体験が設計されている
- [ ] マイクロインタラクションが組み込まれている
- [ ] ブランドの世界観が表現されている
