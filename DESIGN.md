...
> 中黒は `･` (半角中黒 U+FF65) です。見た目の似た `・` (全角中黒 U+30FB) ･ `·` (中点 U+00B7) とは別の文字で、どちらも使いません。

| # | ルール | OK 例 | NG 例 |
|---|---|---|---|
| 1 | 中黒は半角 `･` (U+FF65)、**前後に半角スペース有り** | `Notion ･ Slack ･ Email` | `Notion・Slack` (全角 U+30FB)、`Notion·Slack` (中点 U+00B7)、`Notion･Slack` (スペース無し) |
